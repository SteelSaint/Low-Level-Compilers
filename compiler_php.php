...
$i = 0;
$rwords = array("BEGIN", "CALL", "CASE", "CEND", "CONST", "DO", "DOWNTO", "ELSE", "END", "FOR", "IF", "ODD", "OF", "PROCEDURE", "REPEAT", "THEN", "TO", "UNTIL", "VAR", "WHILE", "WRITE", "WRITELN", "FUNCTION", "AND", "OR", "NOT");	//reserved words, 4 more added
$ops = array("+", "-", "*", "/", "(", ")", "=", ",", ".", "<>", "<", ">", "<=", ">=", ";", ":", ":=");	// operators
...
$st_types = array("CONSTANT", "VARIABLE", "PROCEDURE", "FUNCTION");	// symbol table types Added FUNCTION
$sym;		// current sym
$id;		// current IDENT
$table;		// symbol table
$code;		// program store
...
$codeinx = 0;
$norw = 26;	// number of reserved words Added 4 more words
...
function block($tx, $lev)
{
	...
	while ($sym == "CONST" || $sym == "VAR" || $sym == "PROCEDURE" || $sym == "FUNCTION") #Added Function
	{
		if ($sym == "CONST")
		{
			do
		...
		if ($sym == "FUNCTION") // ADDED FUNCTION
		{
			do
			{
				getsym();
				$tx = funcdelcaration($tx, $lev, $dx);
			}
			while ($sym == "COMMA");
			getsym();
			block($tx, $lev + 1);
...
function statement($tx, $lev)
{
	...
	if ($sym == "IDENT")
	{
		...
		getsym();
		expression($tx, $lev);
		if ($table[$i]->kind == "VARIABLE")		#Added edits for variable only Functions
			gen("STO", $lev - $table[$i]->lvl, $table[$i]->addr);	#Added function modification
		else if ($table[$i]->kind == "FUNCTION")	#We need this for Function
			gen("STO", 0 ,-1);						#Added p code for function
		else 										
			error(12);						#End function modification
	}
	else if ($sym == "CALL")
	{
		getsym();
	...
	else if ($sym == "IF")
	{
		...
		if ($sym !="ELSE")			#Added Else
		{
			getsym();					#Cause Else
			statement($tx, $lev);		
			$code[$cx1]->a = $codeinx;	#End else modification
		}
	}
	else if ($sym == "WHILE")
	{
...
// GeneralExpression
function generalexpression($tx, $lev)	#Changed condition to generalexpression
{
	global $sym;
	$savesym;
...
function expression($tx, $lev)
{
	...
	if ($sym == "PLUS" || $sym == "MINUS" || $sym == "OR")	#Added OR to expression
	{
		$savesym = $sym;
		getsym();
	}
...
function term($tx, $lev)
{
	...
	factor($tx, $lev);
	while ($sym == "TIMES" || $sym == "SLASH" || $sym == "AND")	#Added AND
	{
		$savesym = $sym;
		getsym();
		factor($tx, $lev);
...
function factor($tx, $lev)
{
	...
	if ($sym == "CALL")
	{
		...
		else if ($table[$i]->kind == "FUNCTION")	#Added function modification
		{
			gen("CAL", $lev - $table[$i]->lvl, $table[$i]->addr);
		}											#Ended function modification
		else
			error(23);
	}
	else if ($sym == "IDENT")	#Added for function modification
	{
		$i = position($id, $tx);
		if ($i == 0)
			error(13);
	...
	{
		getsym();
		generalexpression($tx, $lev);       #Added General expression
		if ($sym != "RPAREN")
			error(11);
		getsym();
	}
...
function enter($kind, $tx, $lev, &$dx)
{
	...
	else if ($kind == "FUNCTION")	#Added Function modification
	{
		$table[$tx]->lvl = $lev;
		$table[$tx]->addr = $dx;
		$dx++;
...
function funcdelcaration($tx, $lev, &$dx)	#Added function modification
{
	global $sym;
...
function error($i)
{
	...
	switch ($i)
	{
		...
		case 22:
			...
			break;
		case 23:
			echo "Must be inside Function body"; #Added function error
			break;
		default:
			echo "Unknown error!";
			break;
