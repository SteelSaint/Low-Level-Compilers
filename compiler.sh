...
st_addr=
st_val=
rwords=( BEGIN CALL CASE CEND CONST DO DOWNTO ELSE END FOR IF ODD OF PROCEDURE REPEAT THEN TO UNTIL VAR WHILE WRITE WRITELN FUNCTION OR NOT AND) #Added FUNCTION, OR, NOT, AND
ops=( + - \* / \( \) = , . \<\> \< \> \<= \>= \; : := )
ops_names=( PLUS MINUS TIMES SLASH LPAREN RPAREN EQUAL COMMA PERIOD NE LT GT LTE GTE SEMICOLON COLON BECOMES )
st_types=( CONSTANT VARIABLE PROCEDURE FUNCTION ) #Function added
blank=0
value=
...
code_l=
code_a=
norw=26		# number of reserved words, Added 4 more keywords
...
block() {
	...
	enter() {
		...
		if [ "$1" == CONSTANT ]; then
			...
			st_lvl[$loc]=$lev
		elif [ "$1" == FUNCTION ]; then		#Function added
			st_lvl[$loc]=$lev				#Function definition added
		fi
		return $loc
	}
	...
	statement() {
		...
		generalexpression() {       #changed from condition to general expression
			local tx=$1
			local savesym=
		...
		expression() {
			...
			term() {
				...
				factor() {
					...
					if [ "$sym" == IDENT ]; then
						...
						case "${st_kind[$i]}" in
							...
							PROCEDURE) error 14
							;;
							FUNCTION)  error 14		 #Added Function and error
							;;
						esac
						getsym
						...
						gen LIT 0 $value
						getsym
					elif [ "$sym" == CALL ]; then    #ADDED FOR FUNCTION MODIFICATION
						getsym
						if [ "$sym" != IDENT ]; then
							error 1
						fi
						...
						getsym
						generalexpression $tx       #Changed expression to generalexpression
						if [ "$sym" != RPAREN ]; then
							error 11
						fi
						getsym
				...
				# TERM
				factor $tx
				while [ "$sym" == TIMES ] || [ "$sym" == SLASH ] || [ "$sym" == AND ]; do	#Added AND
					savesym=$sym
					getsym
					factor $tx
			...
			# EXPRESSION
			if [ "$sym" == PLUS ] || [ "$sym" == MINUS ] || [ "$sym" == OR ]; then	#Added OR
				savesym=$sym
				getsym
			fi
			term $tx
		...
		if [ "$sym" == IDENT ]; then
			...
			if [ "${st_kind[$i]}" != VARIABLE ] && [ "${st_kind[$saveKind]}" != FUNCTION]; then	#Added Function
				error 14
			fi
			getsym
			if [ "$sym" != BECOMES ]; then
			...
			elif [ "${st_kind[$saveKind]}" == FUNCTION ]; then   #ADDED FOR FUNCTION MODIFICATION
				gen STO 0 -1
				fi
			fi   #END FUNCTION MODIFICATION
		elif [ "$sym" == CALL ]; then
			getsym
			if [ "$sym" != IDENT ]; then
				error 1
			...
			if [ "$sym" == ELSE ]; then	#Else modification starts
				getsym
				statement $tx
			fi							#Else modification ends
		elif [ "$sym" == WHILE ]; then
			getsym
			cx1=$codeinx
			condition $tx
...
#		elif [ "$sym" == REPEAT ]; then
		...
		elif [ "$sym" == WRITE ]; then	#Write modification starts here
			...
			getsym					#Write modification ends here
		elif [ "$sym" == WRITELN ]; then	#Writeln modification starts here
			...
			getsym			#Writeln modification ends here
#		elif [ "$sym" == FOR ]; then
...
#		elif [ "$sym" == CASE ]; then
	...
	while [ "$sym" == CONST ] || [ "$sym" == VAR ] || [ "$sym" == PROCEDURE ] || [ "$sym" == FUNCTION ]; do	#Added Function modification
		if [ "$sym" == CONST ]; then
			getsym
			constdeclaration $tx
			tx=$?
		...
		while [ "$sym" == PROCEDURE ] || [ "$sym" == FUNCTION ]; do	#Added Function modification
			getsym
			if [ "$sym" != IDENT ]; then
				error 1
			fi
			...
			getsym
				...
				if [ "$sym" == FUNCTION ]; then   #ADDED FOR FUNCTION MODIFICATION
					getsym
					if [ "$sym" != IDENT ]; then
						error 1
					fi
